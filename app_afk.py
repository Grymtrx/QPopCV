...
class QPopApp:
    def __init__(self) -> None:
        ...
        # AFK timer state
        self._afk_seconds: int = 0
        self._afk_alert_sent: bool = False
        ...
        # Slightly taller to fit AFK timer row
        self.root.geometry("350x230")
        self.root.minsize(340, 230)
        ...
        # Start the AFK timer tick loop (it only increments while watching)
        self._schedule_afk_tick()
    ...
    def _build_ui(self) -> None:
        ...
        # Row 4: AFK timer (NEW)
        afk_frame = ctk.CTkFrame(card, fg_color="transparent")
        afk_frame.grid(
            row=4, column=0, columnspan=3, padx=6, pady=(0, 4), sticky="we"
        )

        self.afk_label = ctk.CTkLabel(afk_frame, text="AFK Timer: 00:00")
        self.afk_label.pack(side="left")

        self.afk_reset_btn = ctk.CTkButton(
            afk_frame,
            text="Reset Timer",
            width=90,
            corner_radius=12,
            command=self.on_reset_afk_timer,
        )
        self.afk_reset_btn.pack(side="right", padx=(4, 0))

        # Row 5: Status
        status_frame = ctk.CTkFrame(card, fg_color="transparent")
        status_frame.grid(
            row=5, column=0, columnspan=3, padx=6, pady=(0, 4), sticky="we"
        )
    ...
    def _start_watch(self) -> None:
        ...
        # Reset AFK timer whenever watching starts
        self._reset_afk_timer_state()
    ...
    def _stop_watch(self) -> None:
        ...
        # We don't auto-reset timer here; user can choose to reset manually.
    ...
    def _schedule_afk_tick(self) -> None:
        """Schedule the next AFK timer tick."""
        self.root.after(1000, self._afk_tick)

    def _afk_tick(self) -> None:
        """
        Called every second. Only increments the AFK timer
        while the watcher is actively running.
        """
        if self._watcher is not None and self._watcher.is_running():
            self._afk_seconds += 1
            self._update_afk_label()

            # 25 minutes = 1500 seconds
            if self._afk_seconds >= 25 * 60 and not self._afk_alert_sent:
                self._send_afk_warning_message()
                self._afk_alert_sent = True

        # keep the loop alive
        self._schedule_afk_tick()

    def _update_afk_label(self) -> None:
        minutes, seconds = divmod(self._afk_seconds, 60)
        self.afk_label.configure(text=f"AFK Timer: {minutes:02d}:{seconds:02d}")

    def _reset_afk_timer_state(self) -> None:
        self._afk_seconds = 0
        self._afk_alert_sent = False
        self._update_afk_label()

    def on_reset_afk_timer(self) -> None:
        """Handler for the 'Reset Timer' button."""
        self._reset_afk_timer_state()

    def _send_afk_warning_message(self) -> None:
        """Send the 25-minute AFK warning to the same Discord webhook."""
        webhook_url = self.webhook_var.get().strip()
        user_id = self.user_var.get().strip()
        ...
        mention = f"<@{user_id}>"
        content = (
            f"{mention} AFK 25 minutes. Move your character before 30 min mark "
            f"to prevent blizzards auto log out."
        )
        ...
        try:
            ...
            print("AFK warning sent to Discord.")
        except Exception as e:
            print("Failed to send AFK warning:", e)
    ...
    def _set_update_status(self, text: str, clickable: bool, color: str = "gray") -> None:
        ...
        # no-op if not clickable; handled above
