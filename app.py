...
class QPopApp:
    ...
    @staticmethod
    def _validate_discord_settings(webhook_url: str, user_id: str) -> bool:
    ...
    def on_save(self) -> None:
        ...
        if not self._validate_discord_settings(
            str(self.config["webhook_url"]), str(self.config["user_id"])
        ):
            return
    ...
    def on_test_discord(self) -> None:
        ...
        if not self._validate_discord_settings(webhook_url, user_id):
            return
        ...
        try:
            requests.post(
                webhook_url,
                json={
                    "content": f"{mention} QPop App | Discord Connection Test: Successful ✅"
                },
                timeout=5,
            )
